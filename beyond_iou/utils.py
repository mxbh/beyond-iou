...
def get_contiguous_segments(one_hot_segmentation):
    ...
    segments_tensor = np.stack([scipy.ndimage.label(seg, structure=kernel)[0] for seg in one_hot_segmentation])
    
    return {c:[np.where(seg==l) for l in range(1, seg.max() + 1)] for c,seg in enumerate(segments_tensor)}
...
def erode_mask(mask, width, implementation='fast'):
    ...
    if implementation == 'fast':
        ...
        raise ValueError(f'Implementation has to be one of "exact" and "fast", received: {implementation}!')
